...
const Section1 = ({
  ...
  return <StyledBox id="carouselBox">
      <Carousel spacing="0px" showDots={true} autoPlay={false} showArrow={false} visibleSlides={1} dotClass="carousel-dot" dotColor={palette.primary.main} totalSlides={mainCarouselData.length}>
        {mainCarouselData.map((item, ind) => <Container key={ind}>
            <StyledGrid container>
              <GridItemOne item md={6} sm={6} xs={12}>
                <Box pt={6}>
                  <LazyImage priority width={100} height={100} alt={item.title} src={item.imgUrl} layout="responsive" objectFit="contain" />
                </Box>
              </GridItemOne>
